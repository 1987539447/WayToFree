...
/**
 * TreeMap - 基于红黑树的有序map，使用实现comparable接口的key的比较器或者使用构造指定的比较器
 * 不限容量，不用扩展
 * 红黑树：自平衡二叉树
 *
 * 读取算法复杂度为log（n）
 *
 * 排序基于key的比较（实现Comparable接口）或者指定的比较器Comparator
 *
 * 非线程安全，返回快速失败的迭代器，返回Entry集合
 *
 * TreeSet 底层使用TreeMap实现，只有key,value为Object
 */
    ...
    implements NavigableMap<K,V>, Cloneable, Serializable
{
    ...
    /**
     * 构建空MAP，内置比较器为NULL，所有KEY必须实现Comparable接口
     */
    public TreeMap() {
        comparator = null;
    }
     ...
     * 存入数据：
     * 先检查根，为空则直接存入
     * 否则使用内置比较器或者key的比较器循环查找
     * 比根小查左树，比根大查右树，相同则修改原值，查到NULL为止
     * 新增结点，比根小则添加到左，否则添加到右
     *
     */
